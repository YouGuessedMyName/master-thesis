...
def disjoint_union(F_region: Frame, region: isl.Set, F_not_region: Frame, not_region: isl.Set) -> Frame:
    ...
    return Frame(restricted_F_region.union_min(restricted_F_not_region), F_region.domain, F_region.variables)

def __CmGenCommon(F: Frame, G: FrameSet, M: Model) -> Frame:
    Phi_F = M.Phi(F)
    w, r = G.eqs[0]
...
def CmGenChi(F: Frame, G: FrameSet, M: Model) -> Frame:
    w, _r = G.eqs[0]
    res = __CmGenCommon(F,G,M)
    return disjoint_union(res, w.pw.non_zero_set(), M.Chi(F), w.pw.zero_set())
